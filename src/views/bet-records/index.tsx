  ...
  getcols = () => {
      ...
      return [
        ...
        },
        {
          ...
          dataIndex: 'code',
          width: 180,
          render: (code: string, record: any) => <div className="ctx-inner-wp"><span className={`inlb txt-c code-bg ${ this.getStyle(code) }`}>{code}</span><span className="odd text-orange">{ (parseFloat(record.dyPointDec.split('-')[0]) / record.multiple).toFixed(3) }</span></div>
        },
        {
          title: '期号',
          dataIndex: 'issue'
